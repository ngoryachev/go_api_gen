...
func runTests(t *testing.T, ts *httptest.Server, cases []Case) {
	for idx, caseItem := range cases {
		...
		caseName := fmt.Sprintf("case %d: [%s] %s %s", idx, caseItem.Method, caseItem.Path, caseItem.Query)

		if caseItem.Method == http.MethodPost {
			reqBody := strings.NewReader(caseItem.Query)
			req, err = http.NewRequest(caseItem.Method, ts.URL+caseItem.Path, reqBody)
			req.Header.Add("Content-Type", "application/x-www-form-urlencoded")
		} else {
			req, err = http.NewRequest(caseItem.Method, ts.URL+caseItem.Path+"?"+caseItem.Query, nil)
		}
		...
		if resp.StatusCode != caseItem.Status {
			t.Errorf("[%s] expected http status %v, got %v", caseName, caseItem.Status, resp.StatusCode)
			continue
		}
		...
		data, err := json.Marshal(caseItem.Result)
		json.Unmarshal(data, &expected)

		if !reflect.DeepEqual(result, expected) {
			t.Errorf("[%d] results not match\nGot: %#v\nExpected: %#v", idx, result, caseItem.Result)
			continue
		}
	}
}
